    ...
    public class SqlBulkCopyHelper
    {
        ...
        public static DataTable GetDataTable<T>(List<T> data)
        {
            ...
            data.ForEach(item =>
            {
                ...
                {
                    var value = info.GetValue(item);
                    if (info.PropertyType.ToString().Equals(typeof(Guid)))
                    {
                        dr[info.Name] = (SqlGuid)value;
                    }
                    else
                    {
                        dr[info.Name] = value;
        ...
        public static bool InsertDb<T>(List<T> data, string dataTableName, string connectionString, params string[] excludeFields)
        {
            try
            {
                if (!data.Any()) return false;
        ...
        public static bool UpdateDb<T>(List<T> data, string dataTableName, string connectionString, string updateItemSql, string primaryKey)
        {
            try
            {
                if (!data.Any()) return false;
                ...
                using (var conn = new SqlConnection(connectionString))
                {
                    ...
                    var updateSql = string.Format("update t2 SET {2}  FROM  {0} AS t1,{1} AS t2 WHERE t1.{3} = t2.{3}", tempDataTableName, dataTableName, updateItemSql, primaryKey);
