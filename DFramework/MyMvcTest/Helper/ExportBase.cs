    ...
    public class ExportBase
    {
            ...
            bool exportPdf = false)
        {
            try
            {
                ...
                if (userPath.Equals(string.Empty))
                {
                    return string.Empty;
                }
                #endregion
                ...
                for (var rownum = 0; rownum <30; rownum++)
                {
                    ...
                    if (keyarray.Count() == 1)   //该行只包含一个元素
                    {
                        ...
                        if (valuearray.Length == 3)   //该行只包含一个元素
                        {
                            ...
                            cellValue.SetCellValue(model.GetType().GetProperty(valuearray[1]).GetValue(model, null).ToString());
                        ...
                        if (valuearray.Length == 4)
                        {
                            ...
                            {
                                var list = model.GetType().GetProperty(valuearray[1]).GetValue(model, null) as IEnumerable<object>;
                                if (list != null)
                                {
                                    ...
                                    dicListValue.ForEach(item =>
                                    {
                                        ...
                                        if (mergedColumn > 1)
                                        {
                                            MergedCell(sheet, titleRow.RowNum, titleRow.RowNum, cellcol, cellcol + mergedColumn-1, cellstyle);
                                        }
                                        ...
                                        dicListValueProperty.ForEach(p =>
                                        {
                                            if (p.Key == item.Key)
                                            {
                                                sheet.GetRow(titleRow.RowNum).Height = (short)(100 * int.Parse(p.Value.Split(',')[0]));
                                                sheet.SetColumnWidth(cellcol, DefaultWidth * int.Parse(p.Value.Split(',')[1]));
                                            }
                                        });
                                    ...
                                    {
                                        childRowCount++;
                                        var newrow = sheet.GetRow(rownum + childRowCount) ?? sheet.CreateRow(rownum + childRowCount);
                                        dicListValue.ForEach(info =>
                                        {
                                            ...
                                            if (mergedColumn > 1)
                                            {
                                                MergedCell(sheet, newrow.RowNum, newrow.RowNum, cellcol, cellcol + mergedColumn - 1, contentstyle);
                                            }
                                            dicListValueProperty.ForEach(p =>
                                            {
                                                if (p.Key == info.Key)
                                                {
                                                    sheet.GetRow(newrow.RowNum).Height = (short)(100 * int.Parse(p.Value.Split(',')[0]));
                                                }
                                            });
                                        });
                                    });
                    ...
                    else
                    {
                        for (var colnum = 0; colnum < keyarray.Count(); colnum++)
                        {
                            ...
                            {
                                MergedCell(sheet, colrow, colrow, col + 1, col + valuearray[0].GetInt() - 1, contentstyle);

                            }

                            var margedCol = valuearray[0].GetInt() - 1;
                            for (int i = 0; i < margedCol; i++)
                            {
                ...
                if (exportPdf && ExportPdfFile.Equals("是"))
                {
                    userPath = CreateOfficePdfFrom(userPath);
                }
                if (ExportZipFile.Equals("是") && !exportPdf)
                {
                    userPath = CreateExcelZipFile(userPath, "xls", account);
                }
        ...
        public static void MergedCell(ISheet sheet, int rowstart, int rowend, int colstart, int colend, ICellStyle cellStyle = null)
        {
